    ...
    <div class="card">
        <form method="POST" action="{{ route('admin.articles.update', $article->id) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea id="article_content_textarea" name="content" style="display: none;" required>{{ old('content', $article->content) }}</textarea>
...
// 6. Initialize on page load (support legacy markdown and rich HTML)
function formatInitialContent(content) {
    if (!content) return '';
    // If it already has HTML markup
    if (/<[a-z][\s\S]*>/i.test(content)) {
        return content;
    }
    // If it's legacy markdown with ![alt](url)
    let formatted = content.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt, url) => {
        return `<img src="${url}" alt="${alt}" draggable="true" id="img-${Date.now()}-${Math.random().toString(36).substr(2, 5)}">`;
    });
...
document.addEventListener('DOMContentLoaded', () => {
    if (textarea.value) {
        editor.innerHTML = formatInitialContent(textarea.value);
